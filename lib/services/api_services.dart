...
import 'package:movie_app/models/keywords_model.dart';
...
class ApiServices {
 ...
 Future<KeywordModel> getMoviesKeywords(int movieId) async {
    final endPoint = 'movie/$movieId/keywords';
    final url = '$baseUrl$endPoint$key';
    final response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      print(endPoint);
    ...
    throw Exception('failed to load movies keywords');
    
  }

 Future<Result> getMoviesReviews(int movieId) async {
    ...
    throw Exception('failed to load movies reviews');
